...
// Type: Shooting.BackgroundTouhouStage01C_3D
...
namespace Shooting
{
  internal class BackgroundTouhouStage01C_3D : BaseBackgroundTouhouStage_3D
  {
    public int Height { get; set; }

    public BackgroundTouhouStage01C_3D(StageDataPackage StageData, string textureName)
    ...
    {
      base.Ctrl();
      this.Velocity = 0.2f * (float) Math.Sin((double) (this.Time / 200));
      if ((double) this.OriginalPosition.X < -1000.0)
      {
        PointF originalPosition = this.OriginalPosition;
        double num = (double) originalPosition.X + 1000.0;
      ...
      {
        PointF originalPosition = this.OriginalPosition;
        if ((double) originalPosition.X <= 1000.0)
          return;
        originalPosition = this.OriginalPosition;
        double num = (double) originalPosition.X - 1000.0;
        originalPosition = this.OriginalPosition;
    ...
    public override void SetupVertex()
    {
      this.vertexBuffer.Lock(0, 0, LockFlags.Discard).WriteRange<TexturedVertex>(new TexturedVertex[4]
      {
        ...
        new TexturedVertex(new Vector3(-500f, 1000f, 0.0f), new Vector2(1f, 0.0f)),
        new TexturedVertex(new Vector3(500f, 0.0f, 0.0f), new Vector2(0.0f, 1f)),
