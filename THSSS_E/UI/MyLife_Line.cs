...
namespace Shooting
{
  public class MyLife_Line : Life_Line
  {
    public MyLife_Line(StageDataPackage StageData, string textureName, PointF OriginalPosition)
    ...
    public MyLife_Line(
      ...
      : base(StageData, TxtureObject, OriginalPosition)
    {
      this.size = 16f;
    }
    ...
    public override void Show()
    {
      ...
      {
        TextureObject textureObject = this.TextureObjectDictionary["Life0d"];
      ...
      {
        TextureObject textureObject = this.TextureObjectDictionary["Life"];
        ref PointF local = ref position;
        originalPosition = this.OriginalPosition;
        double num = (double) originalPosition.X + (double) index * (double) this.size;
        originalPosition = this.OriginalPosition;
      ...
      if (this.Life >= 8)
        return;
      int num1 = this.MyPlane.LifeChip * 5 / this.MyPlane.nextLifeChip[this.MyPlane.LifeUpCount];
      if (num1 <= 0)
        return;
      TextureObject textureObject1 = this.TextureObjectDictionary["Life" + (num1 - 1).ToString("D2")];
