...
{
  internal sealed class 无计可施 : AchievementBase
  {
    public override bool Check(Hashtable data)
    {
      if (!((string) data[(object) "name"] == "「藏于牌中的杀意」") || (bool) data[(object) "shoot"] || ((bool) data[(object) "bomb"] || (bool) data[(object) "dead"]))
        return false;
