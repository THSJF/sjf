...
namespace Shooting {
    ...
    public class EventGroup {
        ...
        public void Update(BaseEmitter_CS CS_Emitter) {
            if(CS_Emitter.Time<1) CS_Emitter.Time=1;
            ...
            numArray1[32]=0.0f;
            if(t<1) t=1;
            if((CS_Emitter.Time-CS_Emitter.StartTime+1)%t==0) ++loop;
        ...
        private bool CheckCondition(float[] conditions,Event ev) {
            ...
            bool flag2 = false;
            if(ev.opreator==">") {
                flag1=conditions[ev.contype]>float.Parse(ev.condition)+(double)(loop*addtime);
            } else if(ev.opreator=="<") {
                flag1=conditions[ev.contype]<float.Parse(ev.condition)+(double)(loop*addtime);
            } else if(ev.opreator=="=") {
                flag1=conditions[ev.contype]==float.Parse(ev.condition)+(double)(loop*addtime);
            ...
            if(ev.opreator2==">") {
                flag2=conditions[ev.contype2]>float.Parse(ev.condition2)+(double)(loop*addtime);
        ...
        private float Twopointangle(float x2,float y2,float x1,float y1) {
            float num = (double)x2-x1==0.0 ? (float)Math.Atan((y2-(double)y1)/(x2-(double)x1+0.100000001490116)) : (float)Math.Atan((y2-(double)y1)/(x2-(double)x1));
            if((double)x2-x1<0.0) num+=3.141593f;
            return num;
        }
        public object Clone() {
            EventGroup ev = (EventGroup)MemberwiseClone();
            ev.EventList=new List<Event>();
            EventList.ForEach(x => ev.EventList.Add((Event)x.Clone()));
            return ev;
        }
    }
}
