...
namespace THMHJ
{
  ...
  public class Cover
  {
    ...
    public Cover(float xs, float ys, LayerManager layerm)
    {
      this.rand = new Cover();
      ...
      Vector2 Player)
    {
          ...
          foreach (EventRead result in parentevent.results)
          {
            if (result.special == 4)
            {
              if (result.changevalue == 9)
                result.res = Player.X + Centers.ox - Centers.x;
              if (result.changevalue == 10)
                result.res = Player.Y + Centers.oy - Centers.y;
              if (result.changevalue == 4)
                result.res = MathHelper.ToDegrees(Main.Twopointangle(Player.X + Centers.ox - Centers.x, Player.Y + Centers.oy - Centers.y, this.x - 4f + Centers.ox - Centers.x, this.y + 16f + Centers.oy - Centers.y));
                ...
                if (result.collector == "且")
                {
                  if ((double) this.conditions[result.contype] > (double) float.Parse(result.condition) + (double) (parentevent.loop * parentevent.addtime) & (double) this.conditions[result.contype2] > (double) float.Parse(result.condition2) + (double) (parentevent.loop * parentevent.addtime))
                  {
                    ...
                    if (!result.noloop)
                    {
                      ...
                      coExecution.value = (double) result.rand == 0.0 ? result.res : result.res + MathHelper.Lerp(-result.rand, result.rand, (float) this.Rand(cs.effect && !cs.bomb, cs.bomb));
      ...
      else
      {
        int num1 = 1000;
        int num2 = 0;
        foreach (Barrage barrage1 in layerm.LayerArray[this.parentid].Barrages)
        {
          if (!barrage1.IsLase & barrage1.parentid == this.bindid & !barrage1.NeedDelete)
          {
            if (this.Deepbind)
            {
              if (barrage1.cover != null)
              {
                ...
                barrage1.cover.Update(layerm, cs, Times, Centers, Player);
              }
              else
              {
                barrage1.cover = this.Clone();
            ...
            {
              foreach (Barrage barrage2 in layerm.LayerArray[this.parentid].Barrages)
              {
